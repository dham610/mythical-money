...
function recalcBalances(bets) {
  ...
  [...bets].reverse().forEach((b) => {
    ...
    if (b.isAdjustment || b.isBonus) { if (b.adjustTarget==="p1") p1+=b.adjustAmount; else p2+=b.adjustAmount; return; }
    if (b.mode==="solo") {
...
function ProfileSelector({ p1Name, p2Name, p1Color, p2Color, onSelect }) {
    ...
    <div style={{background:t.bg,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"Georgia,serif"}}>
      ...
      <div style={{textAlign:"center",padding:40}}>
        ...
        <div style={{display:"flex",gap:20,justifyContent:"center"}}>
          {[["p1",p1Name,p1Color.highlight],["p2",p2Name,p2Color.highlight]].map(([key,name,color])=>(
...
export default function App() {
  ...
  const filteredBets = bets.filter((b) => {
    if(b.isAdjustment||b.isBonus) return filterMode==="All"||filterMode==="Adjustments";
    ...
    if(filterMode==="Adjustments"&&!b.isAdjustment&&!b.isBonus) return false;
    return true;
  });
  ...
  const stats={
    total:bets.filter(b=>!b.isAdjustment&&!b.isBonus).length,
    pending:bets.filter(b=>b.result==="Pending").length,
    settled:bets.filter(b=>b.result!=="Pending"&&!b.isAdjustment&&!b.isBonus).length,
    ...
    totalWagered:bets.filter(b=>!b.isAdjustment&&!b.isBonus).reduce((s,b)=>s+(b.amount||0),0),
  };
  ...
  const claimRankingBonus = () => {
    ...
    const adjRecord = { id: Date.now()+1, timestamp: nowTimestamp(), date: new Date().toISOString().slice(0,10), isBonus:true, adjustTarget:player, adjustAmount:bonus, description:"🏆 Power Rankings Bonus — "+wrestler+" ("+divLabel+" Rank #"+(rank+1)+") won: "+titleWon, sport:"WWE", betType:"Bonus", mode:"bonus", result:"Applied", winner:"none", amount:0, p1BalAfter:p1, p2BalAfter:p2 };
  ...
  if (!activePlayer) {
    return <ProfileSelector p1Name={state.p1Name} p2Name={state.p2Name} p1Color={p1Color} p2Color={p2Color} onSelect={selectPlayer} />;
  }
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"22px 20px"}}>
                      ...
                      return (
                        <tr key={bet.id} className="br" style={{borderBottom:`1px solid ${T.border}`}}>
                          ...
                          <td style={{padding:"8px 9px"}}>
                            {isAdj||isBonus?<span style={{color:T.textMuted,fontSize:11}}>&mdash;</span>:
                          ...
                          <td style={{padding:"8px 9px"}}>
                            {isBonus?<span className="pill" style={{background:"#0A2A1A",color:"#5AAF7A"}}>BONUS</span>
                          ...
                          <td style={{padding:"8px 9px",maxWidth:180}}>
                            ...
                            {!isAdj&&!isBonus&&<div style={{color:T.textMuted,fontSize:10,marginTop:1}}>{bet.betType}{isSolo&&bet.payout!==1?` · ${bet.payout}x`:""}</div>}
                          ...
                          <td style={{padding:"8px 9px",fontFamily:"'Bebas Neue',cursive",fontSize:15,color:isBonus?"#5AAF7A":isAdj?"#6FA8DC":T.gold,whiteSpace:"nowrap"}}>
                            {isBonus?(bet.adjustAmount>0?"+":"")+fmt(bet.adjustAmount):isAdj?(bet.adjustAmount>0?"+":"")+fmt(bet.adjustAmount):isPLE?<span style={{fontSize:10,color:T.textMuted}}>see picks</span>:fmt(bet.amount)}
                          </td>
                          ...
                          <td style={{padding:"8px 9px",fontFamily:"'Bebas Neue',cursive",fontSize:13,color:(isAdj||isBonus)&&bet.adjustTarget==="p1"?"#5AAF7A":bet.winner==="p1"?"#5AAF7A":bet.winner==="p2"?"#E06C75":T.textMuted,whiteSpace:"nowrap"}}>
                            {bet.p1BalAfter!=null?fmt(bet.p1BalAfter):"—"}
                          </td>
                          <td style={{padding:"8px 9px",fontFamily:"'Bebas Neue',cursive",fontSize:13,color:(isAdj||isBonus)&&bet.adjustTarget==="p2"?"#5AAF7A":bet.winner==="p2"?"#5AAF7A":bet.winner==="p1"?"#E06C75":T.textMuted,whiteSpace:"nowrap"}}>
        ...
        {tab==="stats"&&(
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            ...
            {bets.filter(b=>b.amount>0&&!b.isAdjustment&&!b.isBonus).length>0&&(
              <div style={{gridColumn:"1/-1",background:T.surface,border:`1px solid ${T.border}`,padding:"16px",borderRadius:3}}>
                <div style={{fontFamily:"'Bebas Neue',cursive",fontSize:16,color:T.gold,letterSpacing:2,marginBottom:12}}>BIGGEST BETS</div>
                {[...bets].filter(b=>b.amount>0&&!b.isAdjustment&&!b.isBonus).sort((a,b)=>b.amount-a.amount).slice(0,5).map(b=>(
                  <div key={b.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:`1px solid ${T.border}`}}>
                    <div><div style={{fontSize:13,color:T.text}}>{b.description}</div><div style={{fontSize:10,color:T.textMuted}}>{b.date} &middot; {b.sport} &middot; {b.mode==="solo"?"Solo":"H2H"}</div></div>
                ...
                <div key={m.name} style={{background:T.surface,border:`1px solid ${m.border}`,borderLeft:`3px solid ${m.color}`,borderRadius:3,padding:"18px 22px"}}>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
                    ...
                    <div style={{display:"flex",gap:8,alignItems:"center"}}>
                      {m.trackPer==="season"&&(<div style={{background:T.surface2,border:`1px solid ${m.border}`,padding:"4px 12px",borderRadius:2,textAlign:"center"}}><div style={{fontSize:9,color:T.textMuted,letterSpacing:1}}>USED THIS SEASON</div><div style={{fontFamily:"'Bebas Neue',cursive",fontSize:20,color:seasonCount>0?m.color:T.textMuted}}>{seasonCount}</div></div>)}
                      {m.trackPer==="player"&&(<><div style={{background:T.surface2,border:`1px solid ${m.border}`,padding:"4px 12px",borderRadius:2,textAlign:"center"}}><div style={{fontSize:9,color:p1Color.highlight,letterSpacing:1}}>{state.p1Name.slice(0,8)}</div><div style={{fontFamily:"'Bebas Neue',cursive",fontSize:20,color:p1Count>0?m.color:T.textMuted}}>{p1Count}</div></div><div style={{background:T.surface2,border:`1px solid ${m.border}`,padding:"4px 12px",borderRadius:2,textAlign:"center"}}><div style={{fontSize:9,color:p2Color.highlight,letterSpacing:1}}>{state.p2Name.slice(0,8)}</div><div style={{fontFamily:"'Bebas Neue',cursive",fontSize:20,color:p2Count>0?m.color:T.textMuted}}>{p2Count}</div></div></>)}
                    </div>
                  </div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:20}}>
            ...
            <div style={{background:T.surface,border:`1px solid ${T.border}`,borderLeft:`3px solid ${T.gold}`,padding:"14px 18px",borderRadius:3}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,fontSize:12,color:T.textMuted,lineHeight:1.7}}>
                ...
                <div>Both players can rank the same wrestler &mdash; no penalty.</div>
                <div>Bonus auto-logs as a Balance Adjustment in the Ledger.</div>
                <div>Rank #1=$20K &nbsp;&middot;&nbsp; #2=$15K &nbsp;&middot;&nbsp; #3=$10K &nbsp;&middot;&nbsp; #4=$5K &nbsp;&middot;&nbsp; #5=$3K</div>
              </div>
            </div>
          </div>
        )}
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              ...
              <div style={{gridColumn:"1/-1"}}><label style={{fontSize:9,color:T.textMuted,letterSpacing:2,display:"block",marginBottom:3}}>BET TYPE</label>
                <select className="fi" value={form.betType} onChange={e=>{const bt=BET_TYPES.find(b=>b.label===e.target.value);setForm({...form,betType:e.target.value,payout:parseFloat(bt?.multiplier)||1});}}>
                  {BET_TYPES.map(b=><option key={b.label}>{b.label} &mdash; {b.multiplier}</option>)}
                </select>
              </div>
        ...
        <div style={{position:"fixed",inset:0,background:T.modalOverlay,display:"flex",alignItems:"center",justifyContent:"center",zIndex:100,padding:20}}>
          <div style={{background:T.modalBg,border:"1px solid #3A1A1A",borderTop:"2px solid #9b1c1c",borderRadius:4,width:"100%",maxWidth:700,maxHeight:"92vh",overflowY:"auto",padding:24}}>
            ...
            <div style={{background:T.surface2,border:`1px solid ${T.border}`,padding:"7px 12px",borderRadius:2,marginBottom:12,fontSize:11,color:T.textMuted}}>
              {pleEntryPlayer==="p1"?`${state.p1Name} is entering picks.`:`${state.p2Name} is entering picks.`}
            </div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              ...
              {adjustForm.amount&&(
                <div style={{background:T.surface2,border:`1px solid ${T.border}`,padding:"9px 13px",borderRadius:2,fontSize:12,color:T.textMuted}}>
                  ...
                  {" "}to {adjustForm.target==="p1"?state.p1Name:state.p2Name}&apos;s stack &rarr;{" "}
        ...
        return(
          <div style={{position:"fixed",inset:0,background:T.modalOverlay,display:"flex",alignItems:"center",justifyContent:"center",zIndex:100,padding:20}}>
            <div style={{background:T.modalBg,border:`1px solid ${T.border2}`,borderTop:`2px solid ${divColor}`,borderRadius:4,width:"100%",maxWidth:460,maxHeight:"92vh",overflowY:"auto",padding:24}}>
              <div style={{fontFamily:"'Bebas Neue',cursive",fontSize:20,color:divColor,letterSpacing:3,marginBottom:2}}>{isMens?"&#x1F4AA;":"&#x1F451;"} {myName.toUpperCase()} &mdash; {divLabel} TOP 5</div>
